...
<Table old_year_children={first_year.child} new_year_children={second_year.child} old_year={selected[0]} new_year={selected[1]}/>
...
<script lang="ts">
    ...
    onMount(() => {
        async function load() {
            const response = await fetch("src/json/revision_budgetpropositionen-for-"+ selected[0] +".json");
            const response2 = await fetch("src/json/revision_budgetpropositionen-for-"+ selected[1] +".json");
