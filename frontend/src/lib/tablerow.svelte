<script lang="ts">
    ...
    <tr>
        ...
        <td style="text-align:center">{(100 - (parseFloat(old_year_children[keys].value.replaceAll(" ", "")) / parseFloat(new_year_children[keys].value.replaceAll(" ", ""))) * 100).toFixed(2)}%</td>
    </tr>
        {#if old_year_children[keys].length !== 0}
            <svelte:self id={id + old_year_children[keys].name.split(" ")[0].toString()+ "."} old_year_children={old_year_children[keys].child} new_year_children={new_year_children[keys].child}/>
        {/if}
    {/each}
